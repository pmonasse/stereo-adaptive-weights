        ...
        for(int d=dispMin; d<=dispMax; d++) {
            ...
            if(xp+d>=0 && xp+d<width) {
                ...
                if(xp+x>=0 && xp+x<width && xp+x+d>=0 && xp+x+d<width) {
                    ...
                    q=distS[dist_s2]*distP[(y+r)*dim+(x+r)];

                    // Combination of weights and raw cost
                    nom+=p*q*dCost(xp+x,yp+y);
                    // normalization term
                    den+=p*q;
                }
